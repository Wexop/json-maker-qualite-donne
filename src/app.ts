...
// Récupérer tous les enfants
  const children = await kysely
    ...
    .where( "parent_station", "!=", "" )
    .select( ["s.stop_name as name", "s.location_type", "r.route_color as color", "s.wheelchair_boarding", "r.route_text_color", "r.route_type as type", "s.parent_station", "r.route_short_name"] )
    .groupBy( "s.stop_id" )
